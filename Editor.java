...
public class Editor extends JFrame {	
	...
	private Point moveFrom = null;				//where teh drawing moves from
	...
	private void handlePress(Point p) {
		if (mode == Mode.DRAW){
			...
			switch (shapeType) {
				...
				case "freehand" -> curr = new Polyline((int) p.getX(), (int) p.getY(), color);
	...
	private void handleDrag(Point p) {
		...
		if(mode == Mode.DRAW){
			switch (shapeType) {
				case "ellipse" -> ((Ellipse) curr).setCorners((int) drawFrom.getX(), (int) drawFrom.getY(), xVal, yVal);
				case "freehand" -> ((Polyline) curr).setEnd(xVal,yVal);
				case "rectangle" -> ((Rectangle) curr).setCorners((int) drawFrom.getX(), (int) drawFrom.getY(), xVal, yVal);
				case "segment" -> ((Segment) curr).setEnd(xVal, yVal);
			}
		}
			...
			if(sketch.containsPoint(p) != null){
				comm.sendMessage("move" + " " + movingId + " " + //send message
						...
						" " + (int) (xVal - moveFrom.getX()) + " " + (int)(yVal - moveFrom.getY()));
				//Shape currShape = sketch.containsPoint(p); //get the shape
				//currShape.moveBy(xVal - (int) moveFrom.getX(),yVal - (int) moveFrom.getY()); //move the shape
				moveFrom = p; //update the moveFrom info
				//repaint();
			}
			repaint();
		}
